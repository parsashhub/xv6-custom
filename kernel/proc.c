...
static void freeproc(struct proc *p) {
  ...
  for (int i = 0; i < NTHREAD; ++i) {
    freethread(&p->threads[i]); // Free all threads associated with the process [cite: 134]
}
}
  ...
  for (;;) {
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      ...
      if (p->state == RUNNABLE) {
        ...
        if (thread_schd(p)) { // Call thread_schd to select a thread for this process
          p->state = RUNNING; // Set process state to RUNNING
          c->proc = p; // Set CPU's current process
          ...
          found = 1; // A process was found and scheduled
        }
      }
      release(&p->lock);
    }
...
struct thread *initthread(struct proc *p) {
  if (!p->current_thread) { // If no current thread exists for the process
      for (int i = 0; i < NTHREAD; ++i) {
          p->threads[i].trapframe = 0; // Clear trapframe pointer
          freethread(&p->threads[i]); // Free any existing thread resources
      }

      // Initialize main thread
      struct thread *t = &p->threads[0];
      t->id = p->pid; // Set thread ID to process ID
      if ((t->trapframe = (struct trapframe *)kalloc()) == 0) {
          freethread(t); // Free resources if trapframe allocation fails
          return 0;
      }
      t->state = THREAD_RUNNING; // Set main thread state to RUNNING
...
struct thread *allocthread(uint64 start_thread, uint64 stack_address, uint64 arg) {
  ...
  if (!initthread(p)) // Ensure the process's main thread is initialized
      return 0;

  for (struct thread *t = p->threads; t < p->threads + NTHREAD; t++) {
      if (t->state == THREAD_UNUSED) {
          t->id = allocpid(); // Assign a unique ID to the new thread
          if ((t->trapframe = (struct trapframe *) kalloc()) == 0) {
              freethread(t); // Free resources if trapframe allocation fails
              break;
          }
          t->state = THREAD_RUNNABLE; // Set thread state to runnable
          *t->trapframe = *p->trapframe; // Copy process's trapframe
          t->trapframe->sp = stack_address; // Set stack pointer for the new thread
          t->trapframe->a0 = arg; // Set the argument for the new thread's function
          t->trapframe->ra = -1; // Return address for thread exit
          t->trapframe->epc = (uint64) start_thread; // Entry point for the new thread
          return t; // Return the newly allocated thread
      }
  }
  return 0; // Return 0 if no unused thread slot is found
}

void freethread(struct thread *t) {
  t->state = THREAD_UNUSED; // Mark thread as unused
  if (t->trapframe)
      kfree((void*)t->trapframe); // Free the trapframe memory
  t->trapframe = 0; // Clear trapframe pointer
  t->id = 0; // Clear thread ID
  t->join = 0; // Clear join information
}

void exitthread() {
  struct proc *p = myproc();
  uint id = p->current_thread->id; // Get ID of the exiting thread

  // Iterate through all threads in the process to wake up joined threads
  for (struct thread *t = p->threads; t < p->threads + NTHREAD; t++) {
      if (t->state == THREAD_JOINED && t->join == id) {
          t->join = 0; // Clear join ID
          t->state = THREAD_RUNNABLE; // Set joined thread to runnable
      }
  }

  freethread(p->current_thread); // Free resources of the current thread

  // If no other runnable threads exist in the process, kill the process
  if (!thread_schd(p))
      setkilled(p);
}
...
int jointhread(uint join_id) {
  ...
  if (!t)
      return -3; // Return error if no current thread

  int found = 0;
  uint current_id = join_id;
  while (current_id != 0) {
      if (current_id == t->id)
          return -1; // deadlock: joining self

      uint target_id = current_id;
      current_id = 0; // Reset for next iteration
  ...
  if (!found)
      return -2; // Target thread not found or already exited

  t->join = join_id; // Set the ID of the thread to join
  t->state = THREAD_JOINED; // Set current thread state to JOINED
...
void sleepthread(int n, uint ticks0) {
  ...
  t->sleep_n = n; // Set sleep duration
  t->sleep_tick0 = ticks0; // Set starting tick for sleep

  t->state = THREAD_SLEEPING; // Set thread state to SLEEPING
  thread_schd(myproc()); // Schedule another thread
}